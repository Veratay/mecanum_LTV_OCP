...
// stale.  The dynamic (HL_TRIG_OCP) solver calls generate_heading_schedule
// each step from the actual x_cur, so it always re-linearizes around the
// true heading.
//
...
int main(int argc, char** argv)
{
    ...
    std::vector<SolverDef> solver_defs = {
        ...
        {"offline_fista", SolveMode::PRECOMPUTED, rerun::Color(220,  80,  80)},
    };
    ...
    constexpr double HEADING_NOISE_STDDEV = 0.1;  // rad (~2.9 deg per step)
